...
def _load_checkpoint_state(base_ckpt: str) -> tuple[dict[str, torch.Tensor], dict]:
  """チェックポイントファイルから state_dict と追加メタデータを抽出する"""
...
class FineTuningNNUE(M.NNUE):
  """
  事前学習済みチェックポイントからファインチューニング用のNNUEモデル
  """
  ...
  def __init__(
    ...
    if incompatible.unexpected_keys or incompatible.missing_keys:
      print(f"Warning: non-strict load. "
            f"unexpected={sorted(incompatible.unexpected_keys)}, "
            f"missing={sorted(incompatible.missing_keys)}")

    if checkpoint:
      if hasattr(self, "on_load_checkpoint"):
